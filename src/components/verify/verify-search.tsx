...
export const VerifySearch = ({ initial = "" }: { initial?: string }) => {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    if (!/^FF-\d{4}-[0-9A-HJ-NP-Z]{8}$/.test(number)) {
      setError("A certificate number looks like FF-2026-7K2M9QX4.");
      return;
    }
    setError(null);
  ...
  return (
    <form onSubmit={submit} className="flex flex-col gap-3 sm:flex-row sm:items-end">
      <Field label="Certificate number" htmlFor="certificate-number" error={error ?? undefined} className="flex-1">
        <Input id="certificate-number" value={value} onChange={(e) => setValue(e.target.value)} placeholder="FF-2026-7K2M9QX4" autoCapitalize="characters" autoComplete="off" spellCheck={false} className="code" />
      </Field>
